...
<?php include(G5_THEME_PATH)."/headForPicture.php"; ?>
  <p class="h1 text-center mt-2 mb-2">Blog & Youtube</p>
  <div class="container mt-3">
    <ul class="nav nav-tabs">
      <li class="nav-item">
        <a class="nav-link notice" href="<?php echo G5_BBS_URL; ?>/board.php?bo_table=gallery;?>">Blog</a>
      </li>
      <li class="nav-item">
        <a class="nav-link qa" href="<?php echo G5_BBS_URL; ?>/board.php?bo_table=<?php echo $bo_table; ?>">Youtube</a>
      </li>
    </ul>
  </div>

<!-- 게시판 목록 시작 { -->
<div class="container" id="bo_gall" style="width:<?php echo $width; ?>">
    <?php if ($is_category) { ?>
    <nav id="bo_cate">
    ...
    <?php } ?>
    <ul id="gall_ul" class="text-center">
        <?php for ($i=0; $i<count($list); $i++) {
         ...
         <?php $board['bo_gallery_width'] = 270;
              $board['bo_gallery_height'] = 250;?>

        <li  style="<?php echo $style ?>width:<?php echo $board['bo_gallery_width'] ?>px; float:left;margin:0 10px 30px 0">
            <?php if ($is_checkbox) { ?>
            <label for="chk_wr_id_<?php echo $i ?>" class="sound_only"><?php echo $list[$i]['subject'] ?></label>
            <input class="float-right" type="checkbox" name="chk_wr_id[]" value="<?php echo $list[$i]['wr_id'] ?>" id="chk_wr_id_<?php echo $i ?>">
            <?php } ?>
                ...
                <li class="gall_href">
                    ...
                    <?php } else {
                      ...
                      if($thumb['src']) {
                        ...
                        $img_content = '<img src="https://img.youtube.com/vi/'.$youtube_key.'/mqdefault.jpg"
